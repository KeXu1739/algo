...
def note():
    ...
    # 链表partition leetcode 86（给一个值，链表小于这个值的放左边，等于放中间，大于放右边）,创六个变量表示小等大区域的头和尾，然后打乱原来链表的指针指向，这个题的边界条件很神奇要自己写一下
    # leetcode 138,复制带有随机节点的链表 o(n) s, o(n) t, and * o(1) s, o(n) t,这个思路比较神奇一定要写一下 <- o(n) done, o(1) 重听一下有点忘了
    # 新节点串在老节点下一个，一对一对遍历，* 新节点->rand = 原节点->rand->next, 最后遍历一遍把新老链表分开（原来老节点的next指向下一个老节点，原来新节点的next指向下一个新节点）
    # 查一个链表有无环，有的话返回第一个入环节点，否则返回None -> leetcode 142，<- o(n) done, o(1): 快慢指针，两个指针相交时，快指针回到头，
    # 两指针从此开始一次走一步，相交时为入环节点 <- o(1) done
    # 查两个链表（可能有环）是否相交，返回相交的第一个节点：使用有无环查找的code分别过一遍两个链表，返回两个链表各自第一个入环节点，然后讨论，一共（考虑有无环，有无相交）
    # 有四种情况 (找找是不是leetcode原题目:leetcode 160 为无环简易版，完成了空间o(n)与o(1))
    ################################ 二叉树 ############################################################################
    ...
    # 最终顺序为左右头
    # 中：打印顺序：左头右，压栈顺序：有左就左,从当前根节点出发的所有左节点全进去，pop并打印，pop的时候如果当前点有右节点，打印右节点，并以右节点为根，有左就左压栈重复这个过程
    # 二叉树DFS: 先序遍历
    # 二叉树BFS: 层序遍历，头左右，使用队列。每一层处理完之后队列里是下一层的节点数
    # leetcode 102, 此题可以衍生出求二叉树最大宽度（即每层最多有多少个非空的node），只需要多加一个变量记录全局最大的宽度
    # 判断 二叉树为：
    # 搜索树 leetcode98: 中序遍历每个元素不降序，则为搜索二叉树; 利用中序遍历的思想和一个全局变量来记录目前为止左边的值, 每个点需要保证左边为inorder
    # 左边最后一个value小于当前node value, 然后右树inorder
    ...
    # 布隆过滤器，大量数据(eg 100亿个url黑名单)，查找某个元素是否在名单中，爬虫去重etc,使用空间很小（可以放在memory里面），但是会有一定误报概率
    # 原本是黑名单里的误报成白的这种不会发生，而会发生原本是白名单里的误报成黑名单，即更加严格
    ...
    # 的哈希值会均匀地插在环上，当有新数据进来后，只需要通过hash函数算一个值，放在顺时针先碰到的那个哈希值对应的机器里就行(记录一个各个机器的hash值
    # array即可)，新加机器的时候就是在环上多个点，大部分的数据不需要重新hash，只需要动一部分机器即可。
    ...
    # 来到当前节点，当前节点有左树,找左树最右节点，a.此节点右指针指向空时，让此节点右指针指向当前节点（即找到当前节点的前驱节点），然后当前节点向左移动；
    # b. 此节点右指针指向当前节点时，让其指向空，当前指针向右移动
    ...
    ######################################################## 中级5 ##########################################################
    # 最小栈问题：实现一个栈，在基本栈的功能基础上加上一个返回栈中最小值的操作：弄两个栈，第一个栈为原始栈，第二个栈和第一个栈同步压入值，每次压进去的新值小于第二个栈顶，则第二个栈顶压入新值，否则第二个栈重复压入栈顶
    # pop的时候两个栈也同时pop，这样就  可以保持第二个栈栈顶元素永远是当前栈中最小的
    # 只用栈，实现队列，弄两个栈，一个负责push一个负责pop，每次pop之前看pop栈是否为空，为空就把当前push栈里的东西倒进pop，然后弹出，不为空就直接弹出 
    # 只用队列实现栈：弄两个队列，其中一个保持为空，另一个进数据，当需要弹出的时候把有数据的栈里面的n-1个元素弹出到另一个对列里然后返回最后一个值；进数据的时候永远进到那个不空的队列里
    # 动态规划二维表空间压缩技巧：当每个位置只依赖其左方和上方（或右方和下方）值时，二维表可以压缩成长度为原来表列数的一维数组
    # 接水问题：给一个arr，每个元素非负，整个数组构成的直方图为容器形状，问容器可以装多少的水，单独考虑每个位置左右两端的最大值，较小的那边和当前的高度决定当前位置可以装多少水，遍历这个arr把每个位置水量加起来
    # 就是最后的结果，可以初始化两个辅助数组计算每个点左和右的最值，也可以用有限个变量记录左右边界和最值，根据左右哪边的最值更小，可以从右往左或从左往右更新
    # 给定一个数组arr长度为N，你可以把他砍成两半，每一个部分必须含有元素，这样左右部分分别有各自的最大值，请返回最大的abs(l_max - r_max): return max(arr) - min(arr[0], arr[-1])
    # string旋转词问题，把string复制一份放在后面，任何旋转词一定是这个长string的子string
    # 咖啡杯问题：arr[]每个元素为正，代表咖啡机效率，n代表多少人喝咖啡，每人喝一杯；a代表唯一一台洗咖啡杯的机器洗一个杯子的时间；b咖啡杯不洗自然变干净需要的时间，问从第一个人泡咖啡开始到最后一个人
    ...
    # 牛牛找工作问题： 收集每一种工作的难度和报酬，找的标准是在难度不超过自身能力的情况下找报酬最高的，给一个job类数组，每个元素里有报酬和难度，给一个能力值array，
    # 假设每个工作有无数个岗位，返回一个报酬数组代表每个能力值获得的报酬 ->sort：难度从小到大，如果难度一样，报酬从大到小排序，之后删掉难度增加但是报酬没增加的记录
    # 然后根据能力值查找
    ########################################################### 中级7 #########################################################
    ...
    # 每次判断：左树是否为搜索树，右树是否为搜索树，若左右均为搜索子树，则判断左边最大，当前节点，和右边最小的相对关系，满足搜索树条件则当前节点info中子树节点个数更新为左+1+右，
    # 若任意一个不为子树则返回左右更大的节点数+1，若左右均非子树则返回1，代表只有当前根节点组成的二叉搜索树；如果要同时返回子树的根节点，则info中需要额外包含一个最大搜索二叉子树头节点
    # 给先序，中序遍历数列，返回后序遍历数列
    ...
    # 活动和收益问题:給一堆具有前后顺序的活动，每个活动具有消耗的时间和活动收益，你可以从任意一个活动开始，根据依赖关系做到最后一个活动，（最后一个活动是固定的而且一定要做到）
    # 问给定我拥有的最大天数，返回我能获得的最大收益以及选择的活动 -> 从最后一个活动向前bfs，每一个活动代表一个node，node里面存一张表，表的key是从当前点出发做到最后一个
    # 活动需要的天数，value为对应的累积收益，在bfs的过程中更新每个节点的这张表，之后合并所有的表，并删掉不合理的记录（删掉耗费天数高但收益低的记录），之后根据自己有的天数
    # 来选择这张大表里的对应活动 =>实现的时候：做成双向图而不是单向图保证可以从最后一个活动往前面bfs
    ...
    # 第一步建立词频表， 从左到右找到最靠右的位置，删掉这个位置左侧所有东西之后剩下的东西至少包含每个字符一次，记这个位置为i，选择从0到i这个范围上字典序最小的最左侧的字符记为K，位置为j,删掉j左侧所有东西，
    ...
    # case2：如果arr[i]位置的值属于最优划分下最后一个xor和为0的子数组，则dp[i] = dp[k] + 1, k为离i最近的xor cumsum和xorsump[i]相同的位置，
    # 这个东西用hashmap来记录，key为xorsum,value为上一个出现这个xorsum的位置
    #################################################################### 高级2 ##########################################
    ...
    # 部分和小的中点的右半部分看作两个等长的有序数组递归找上中位数，子问题的结果就是最终大问题的结果;长度为奇数时：先看各自中点位置的数，如果相等直接返回，
    # 这个数就是上中位数，否则，大的那个数包含自己和自己序列中与自己大的数都有可能，以及小的那个数自己序列中前面剩下的数都有可能，
    ...
    # 老=(新+m-1) % i + 1
    #################################################################### 高级4 ##########################################
    # 有序表：Key按有序的方式组织起来的结构，实现：红黑树，跳表，AVL树，size-balance tree，时间复杂度上没有区别，有序表增删改查全是log(n)复杂度
    ...
    # 给一个数组arr，无序，但每个值都是正数，再给另一个正数target，求arr所有子数组中元素和相加为target的最长子数组长度，需要时间复杂度O(N),空间O(1) 
    # -> 两个指针的滑动窗口
    # 给一个数组arr，无序，每个值可能正负0，再给另一个正数target，求arr所有子数组中元素和相加为target的最长子数组长度，需要时间复杂度O(N),空间O(1) 
    ...
    # 给定一个非负数组，每个值代表该位置上有几个铜板，a和b玩游戏，a先手，b后手，轮到某个人的时候他可以在某一个位置拿任意数量的铜板，但是不能不拿（不能拿0个），谁先把铜板拿完谁获胜，a和b都绝顶聪明，问谁获胜
    # xor所有的元素，如果非0那么先手赢，否则后手赢 -> 组合博弈中的尼姆博弈问题，谁先面对异或和为0的状态谁输，从非0异或和状态一定可以转化为0异或和状态
    ############################################################################## 高级5 ################################
    # excel 列号转数字和数字转列号问题
    ...
    # 给定两个字符串str1和str2，求两个字符串的最长公共字串（字串是连续的）dp[i][j]:str1字串必须以i结尾，str2字串必须以j结尾时最长公共子串长度，且最长公共子串也必须以str1[i],str2[j]结尾
    # 根据dp含义，表中任何一个行列位置，只要str1[i]!=str2[j]则dp[i][j]为0，相等则只依赖dp[i-1][j-1]的值,整个表中最大值即为最长公共字串长
    # 给定两个字符串str1和str2，求两个字符串的最长公共子序列（子序列可以是不连续的）dp[i][j]:str1到达位置i时子序列，str2到达j时子序列最长公共子序列长度，且最长公共子序列不一定以str1[i],str2[j]结尾
    # 看代码，这个根子串的思路不太一样，dp的定义也有区别
    ############################################################################## 高级6 ###############################################################################
    # 给定一个数组arr，长度为N且每个值都是正数，代表N个人的体重，再给定一个正数limit,代表一艘船的载重，坐船规则是一艘船最多坐两个人，且体重和不能超过limit，返回同时让N个人渡河最少几艘船
    # 给定一个字符串str返回最长的回文子序列: -> 把原始串逆序，然后求原始串和逆序串的最长公共子序列即为最长回文子序列
    # 范围上尝试模型：讨论开头和结尾位置存在不存在的可能性来进行尝试。
    # 这个问题子问题:dp[i][j] = str[i...j]上最长回文子序列是多长,讨论开头和结尾的情况共四种：a.str[i...j]上最长回文子序列不以i开头不以j结尾, b.str[i...j]上最长回文子序列以i开头不以j结尾
    # c.str[i...j]上最长回文子序列不以i开头以j结尾, d.str[i...j]上最长回文子序列以i开头以j结尾
    # a: dp[i][j] = dp[i+1][j-1], b: dp[i][j] = dp[i+1][j], c: dp[i][j] = dp[i][j-1], d: dp[i][j] = dp[i+1][j-1]+2,每个格子依赖自己下方，左方和左下方的格子，从右下角向左上角填或者从左上角向右下角填，
    # 选择abcd四种结果中最大的值
    # 其中对角线和两字符相邻的情况是base case，右上角为最终答案代表从0到len-1范围上最长回文子序列的长度
    # 给定一个字符串str，如果可以在str的任意位置添加字符，请返回在添加字符最少的情况下，让str整体都是回文字符串的其中一种结果
    # dp[i][j]=str[i...j]需要至少添加几个字符使得其变为回文，一般情况有三种: a.单独拿出str[i]位置文字考虑剩下的部分先变成回文，然后在最后添加str[i], dp[i][j] = dp[i+1][j] + 1
    ...
    # 给定一个字符串str，可以通过移除其中一部分来使之变为回文串，(移除0个或多个字符)，如果空串不被认为是回文串，问一共有多少种移除方法可以使str变为回文串？若串中移除的两个字符相同但是位置不同，则认为是两种不同的移除方案。
    ...
    # 对于1，如果i和j位置字符不同，则没有1的方案可能性，若i和j位置相同，则可能方案是在dp[i+1][j-1]基础上最左最右均加上相同的字符，或者只保留i和j， 则这种情况为dp[i+1][j-1]+1,其中加的1代表只保留i和j，dp[i+1][j-1]代表以dp[i+1][j-1]基础上最左最右均加上相同的字符
    # dp[i][j] = dp[i+1][j] + dp[i][j-1] - dp[i+1][j-1] if str[i] != str[j] else dp[i+1][j] + dp[i][j-1] - dp[i+1][j-1] + (dp[i+1][j-1] + 1)
    #################################################################################### 高级7 ##########################################################################################
    # 在一个无序数组中求最小的第K个数
    # BFPRT算法:无随机严格O(N) f(arr, K)
    # 1. 五个数一组分组 2.求每5个数的中位数，构成数组marr，O(N) 3. f(marr, len(marr)/2) = X, 4.以X为分界做荷兰国旗问题 5.如果X hit到了第K小直接返回，否则剩余问题规模最大为0.7N
    # T(N) = T(0.7N) + T(N/5) + O(N) -> T(N)为严格的O(N)复杂度
    # 给定一个正数N，返回裂开这个数的方法数，裂开的每个数最小为1，假设N裂开成三个不同的数，要求第二个数不小于第一个数且第三个数不小于第二个数
    # 给定一颗二叉树的头节点head，已知所有的节点值都不一样，返回其中最大的且满足二叉搜索树条件的拓扑结构有几个节点。拓扑结构不是二叉搜索子树，只要能连起来都算
    # 和中级的那个最大搜索二叉子树节点类似，每个info提供以当前节点为头的树可以为整体结构贡献多少个满足BST的节点
