...
def note():
    ...
    # 接雨水问题：给一个arr，每个元素非负，整个数组构成的直方图为容器形状，问容器可以装多少的水，单独考虑每个位置左右两端的最大值，
    ...
    #################################################### 高级8 ##########################################################
    # 判定一个由[a-z]字符组成的字符串和一个包含[a-z]，?和*通配符的字符串是否匹配，?匹配任意单一非空字符，*匹配任意多个字符包括0个字符，字符串长
    # 度不超过100。*的用法： 不能独立使用，a*可以匹配aaaa....任意长度的只含a的串(或空串，代表0个a)
    ...
    ###################################################### 高级9 #######################################################
    # 一个字符串可以分解成多种二叉树结构，如果str长度为1， 认为不可分解，如果str长度为N (N>1),左部分长度可以分为1~N-1,剩下的为右部分长度，左
    # 部分和右部分都可以按照相同的逻辑继续分解，形成的所有结构都是str的二叉树的结构，旋变串：产生的结构中任意一个节点的左右节点构成一组兄弟组，
    # 任何一个兄弟组的左右可以交换，交换后可以结合成一个新的字符串，叫做原始串的旋变串，给两个字符串str1和str2，请返回这两个串是否互为旋变串
    # 两个串上范围上尝试，str1[L1...R1], str2[L2...R2]能否互为旋变串->四维表，由于长度必须一样，可以用str1[L1...L1+k]和str2[L2...L2+k]
    # 变成三维
    # 给定字符串str1和str2，求str1的子串中含有str2所有字符的最小子串长度 （最小覆盖子串问题）
    # y一个缓存结构需要实现以下功能：.set(key,value),加入或修改对应key的value，.get(key),查询key对应的value，但是缓存中最多放K条记录，如果
    # 新的K+1条记录要加进来，需要根据策略删掉一条记录，然后才能把最新的记录加入，这个策略为，在缓存j结构现在的K条记录中，哪一个Keyc从进入缓存
    # 结构开始，被调用set或者get的次数最少，就删掉这个key的记录，如果调用次数最少的key有多个，上次调用发生最早的key被删除，这叫做LFU缓存算法
    # (Least Frequently Used Cache)
    # 解法是使用一个二维双向链表结构，其中一个维度(桶)用来装每个操作词频的节点们，另一个维度为相同的操作词频装先后来的节点，然后创一个hashmap来装key
    # ->节点地址，用于快速查到节点，每次操作时，找到节点，操作数+1，看有没有下一个操作频率的桶，有的话把节点从当前的桶拿出来放在下一个桶的结尾，如果
    # 没有下一个频率的桶就新建一个然后把节点放进去，删除同理，这可以保证这个结构所有操作都是O(1)的，每次LFU替换的时候删掉最前面的桶的头节点即可
    # 环形加油站问题...这个是绿皮上的quant问题，这里用代码实现，N个加油站组成一个环形，给定两个长度都是N的非负数组oil 和dis，oil[i]代表第i个
    # 加油站存的油可以跑多远，dis[i]表示第i个加油站到环中下一个加油站相隔多远，假设你有一辆油箱无限大的车，刚开始时车没有油，如果车从第i个加油站出发，
    # 最终可以回到这个加油站，那么这个加油站就算良好加油站，否则不算，请返回长度为N的res，res[i]代表第i个加油站是不是良好加油站
    # 连通区边界，l,r, 连通区的net oil -> rest, 当前节点接上连通区的l点需要至少多少油。从任何一个正数点逆时针出发，试图向右以及向左扩连通区
    # 分为找到完整连通区和找不到完整连通区两种情况，如果可以找到完整连通区，l部分向左扩时只要新的点可以到达连通区的头就是良好点，如果找不到完整连通区，
    # 则连通区包含的点都不是良好出发点
    ######################################################## 高级10 #####################################################
    # 一棵二叉树原本是搜索二叉树，但是其中两个节点调换了位置，使得这个二叉树不再是搜素二叉树，请找到这两个错误节点并返回
    # 中序遍历，第一个降序的前一个和最后一次降序的后一个是需要换的东西，如果只有一次降序则他既是第一次也是最后一次
    # 进阶：如果在原问题中得到了这两个错误节点，希望你通过调整节点结构让整棵树变回搜索二叉树，请实现调整的函数
    # 同样中序遍历找出两个点，然后考虑情况 1. e1 e2谁是根节点 2.e1 e2相邻且其中一个是另一个的父 3.e1 e2有各自的父，他们是左子还是右子
    # 平面内有n个矩形，第i个矩形左下角坐标为(x1[i],y1[i]),右上角为(x2[i],y2[i]),如果两个或多个矩形有公共区域认为他们s是相互重叠的(不考虑
    # 边界和角落),请计算出平面内重叠矩形最多的地方有多少个矩形互相重叠。
    # 类似问题：最多的覆盖线段问题，给一些数轴上线段的起点和终点，返回最多有多少重合线段(只有边界相等认为不重合)
    # 先按照起点排序，按起点从小到大loop，每到一个线段，在有序表中删掉终点小于当前线段起点的记录,然后把当前线段结尾放在有序表中，处理完成后有序表
    # 里有几个元素即为目前重叠的区域数量，然后一直循环并更新最大的覆盖数  -> 以当前线段开头的线段最多盖多少#
    pass
